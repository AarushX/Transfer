...
<section class="space-y-5">
	<div class="fade-up">
		...
		<h1 class="text-2xl font-bold tracking-tight">
			<span class="gradient-text">Parent Application Approvals</span>
		</h1>
		<p class="mt-1 text-sm" style="color: var(--app-text-muted);">
			Approve parent onboarding course submissions before they can link students and access
			forms/carpool.
		</p>
	</div>
	{#if form?.error}
		...
		{#each data.applications as app}
			...
			>
				<div class="flex flex-wrap items-center justify-between gap-2">
					<div>
						<p class="font-semibold" style="color: var(--app-text);">
							{app.parent?.full_name || app.parent?.email || app.parent_user_id}
						</p>
						<p class="text-xs" style="color: var(--app-text-muted);">
							{app.parent?.email} · {app.relationship || '—'} · {app.phone || '—'}
						</p>
					</div>
					<div class="flex items-center gap-2">
						<span
							class="rounded-full border px-2.5 py-1 text-[11px] font-semibold tracking-wider uppercase"
				...
				>
					...
					{:else}
						<p class="mt-1 text-sm" style="color: var(--app-text-muted);">
							No completed parent-application course yet.
						</p>
					{/each}
				</div>
