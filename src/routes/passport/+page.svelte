		...
		>
			...
			<div class="relative space-y-4">
				...
				<div class="flex flex-wrap items-center gap-2">
					<span
						class="chip-amber inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium"
					>
					...
					{#if specialTitles.length > 0}
						{#each specialTitles as title, i}
							<span
								class="chip-emerald fade-up inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium"
				...
				<div>
					...
					<div class="mt-2 space-y-2">
						{#each trackRanks as rank, i}
							<div
								class="fade-up relative overflow-hidden rounded-xl border p-3 backdrop-blur-xl"
								style="background: var(--app-glass-bg); border-color: var(--app-glass-border); box-shadow: var(--app-glass-shadow); animation-delay: {0.05 *
									(i + 1)}s"
							>
								<div
								...
								<div class="relative flex items-center justify-between">
									...
									<div class="flex items-center gap-2">
										<span
											class="chip-violet inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-medium"
											>{rank.tier}</span
										>
										<span class="mono text-xs" style="color: var(--app-text-dim);"
											>{rank.count} completed</span
										>
									</div>
								</div>
							</div>
						{:else}
							<p class="text-sm" style="color: var(--app-text-dim);">
								Complete courses to earn track tiers.
							</p>
						{/each}
					</div>
				</div>
				...
				<div>
					...
					<div class="mt-2 flex flex-wrap gap-2">
						{#each data.badges as badge, i}
							<span
								class="chip-cyan fade-up inline-flex items-center rounded-full border px-2.5 py-1 text-xs font-medium"
								style="animation-delay: {0.05 * (i + 1)}s"
							>
								{badge}
							</span>
