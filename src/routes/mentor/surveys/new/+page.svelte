	...
	let questions = $state<SurveyQuestion[]>([
		{
			...
			type: 'short',
			correct: '',
			short_ignore_case: true,
			short_ignore_punctuation: false
		}
	]);
...
<section class="space-y-5">
	...
	>
		{#if (data.templates ?? []).length > 0}
			...
			>
				{data.templates.length} saved template{data.templates.length === 1 ? '' : 's'} available for future
				expansion.
			</div>
		{/if}
		<div
		...
		<div class="flex flex-wrap gap-4 text-sm" style="color: var(--app-text);">
			...
			>
			<label class="inline-flex items-center gap-2"
				><input type="checkbox" name="show_when_inactive" /> Show to users when inactive</label
			>
			<label class="inline-flex items-center gap-2"
				><input type="checkbox" name="allow_student_view_submissions" checked /> Allow students to view
				submissions</label
			>
			<label class="inline-flex items-center gap-2"
				><input type="checkbox" name="allow_student_edits" /> Allow student edits</label
			>
		</div>
		...
		>
			<label class="inline-flex items-center gap-2 text-sm" style="color: var(--app-text);">
				<input type="checkbox" name="save_as_template" bind:checked={saveAsTemplate} /> Save this as a
				template
			</label>
			{#if saveAsTemplate}
				<input
