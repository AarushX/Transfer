...
<section class="space-y-5">
	...
	<form method="POST" action="?/saveSurvey" class="fade-up space-y-3 rounded-2xl border p-5 backdrop-blur-xl" style="background: var(--app-glass-bg); border-color: var(--app-glass-border); box-shadow: var(--app-glass-shadow); animation-delay: 0.05s;">
		...
		<div class="grid gap-2 md:grid-cols-2">
			<label class="flex flex-col gap-1 text-sm"><span style="color: var(--app-text);">Visible from (optional)</span><input type="datetime-local" class={gi} style={gs} name="visible_from" value={dateValue(survey.visible_from)} /></label>
			<label class="flex flex-col gap-1 text-sm"><span style="color: var(--app-text);">Visible until (optional)</span><input type="datetime-local" class={gi} style={gs} name="visible_until" value={dateValue(survey.visible_until)} /></label>
		</div>
		<div class="flex flex-wrap gap-4 text-sm" style="color: var(--app-text);">
			...
			<label class="inline-flex items-center gap-2"><input type="checkbox" name="show_when_inactive" checked={survey.show_when_inactive} /> Show to users when inactive</label>
			<label class="inline-flex items-center gap-2"><input type="checkbox" name="allow_student_view_submissions" checked={survey.allow_student_view_submissions} /> Allow students to view submissions</label>
			<label class="inline-flex items-center gap-2"><input type="checkbox" name="allow_student_edits" checked={survey.allow_student_edits} /> Allow student edits</label>
		</div>
		<label class="flex flex-col gap-1 text-sm md:max-w-sm"><span style="color: var(--app-text);">Student edit deadline (optional)</span><input type="datetime-local" class={gi} style={gs} name="student_edit_deadline" value={dateValue(survey.student_edit_deadline)} /></label>
		<div>
			...
			<div class="grid max-h-52 gap-1 overflow-auto rounded-lg border p-2" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
				{#each data.nodes as node (node.id)}
					<label class="inline-flex items-center gap-2 text-sm" style="color: var(--app-text);"><input type="checkbox" name="prereq_node_ids" value={node.id} checked={selectedPrereqs.has(node.id)} /> {node.title}</label>
				{/each}
			</div>
		</div>
		<div class="space-y-2">
				...
				<div class="space-y-2 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
					...
					{#if q.type === 'mc'}
						{#each q.options ?? [] as _o, oi (oi)}
							<div class="flex items-center gap-2"><input type="radio" name={`mc-${idx}`} checked={q.correct === q.options?.[oi]} onchange={() => (q.correct = q.options?.[oi] ?? '')} /><input class={"flex-1 " + gi + " py-1 text-sm"} style={gs} bind:value={q.options![oi]} placeholder={`Option ${oi + 1}`} /></div>
						{/each}
					...
					{:else if q.type === 'ms'}
						{#each q.options ?? [] as _o, oi (oi)}
							<div class="flex items-center gap-2"><input type="checkbox" checked={Array.isArray(q.correct) && q.correct.includes(q.options![oi])} onchange={(e) => { const checked = (e.currentTarget as HTMLInputElement).checked; const current = Array.isArray(q.correct) ? q.correct : []; const option = q.options![oi]; q.correct = checked ? Array.from(new Set([...current, option])) : current.filter((v) => v !== option); }} /><input class={"flex-1 " + gi + " py-1 text-sm"} style={gs} bind:value={q.options![oi]} placeholder={`Option ${oi + 1}`} /></div>
						{/each}
		...
		<div class="flex gap-2">
			...
			<button type="submit" formmethod="POST" formaction="?/deleteSurvey" class="inline-flex items-center justify-center gap-1.5 rounded-lg px-3.5 py-2 text-sm font-medium" style="background: color-mix(in srgb, var(--app-danger) 85%, transparent); color: white; border: 1px solid color-mix(in srgb, var(--app-danger) 50%, transparent);">Delete</button>
		</div>
	</form>
	...
	<GlassCard>
		...
		<div class="mt-3 grid gap-3 md:grid-cols-3">
			...
			<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
				...
				<p class="text-xl font-semibold" style="color: var(--app-text);">{responseQuestions.length}</p>
			</div>
			<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
		...
		<div class="mt-3 flex flex-wrap gap-2">
			{#each [['summary', 'Summary'], ['question', 'Question view'], ['individual', 'Individual responses']] as [view, label]}
				<button
					...
					style={responsesView === view ? 'border-color: var(--app-accent); background: color-mix(in srgb, var(--app-accent) 12%, transparent); color: var(--app-text);' : 'border-color: var(--app-glass-border); color: var(--app-text);'}
					onclick={() => (responsesView = view as any)}
				>{label}</button>
			{/each}
		</div>
		...
		{:else if responsesView === 'summary'}
			<div class="mt-3 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
				...
				<div class="space-y-2">
					{#each submissions.slice(0, 10) as submission (submission.id)}
						<div class="flex flex-wrap items-center justify-between gap-2 rounded-lg border px-3 py-2 text-sm" style="border-color: var(--app-glass-border); background: color-mix(in srgb, var(--app-glass-bg) 60%, transparent);">
							<p style="color: var(--app-text);">{submission.profile?.full_name || submission.profile?.email || submission.user_id}</p>
		...
		{:else if responsesView === 'question'}
			<div class="mt-3 space-y-3 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
					...
					{#if selectedQuestion.type === 'short'}
						<div class="space-y-2">
							{#each questionSummaryRows as row, i (i)}
								<div class="rounded-lg border p-2 text-sm" style="border-color: var(--app-glass-border); background: color-mix(in srgb, var(--app-glass-bg) 60%, transparent); color: var(--app-text);">
									{row.label}
								</div>
							{/each}
						</div>
					{:else}
						<div class="space-y-2">
							{#each questionSummaryRows as row (row.label)}
								<div class="space-y-1 rounded-lg border p-2" style="border-color: var(--app-glass-border); background: color-mix(in srgb, var(--app-glass-bg) 60%, transparent);">
									<div class="flex items-center justify-between text-sm">
										<p style="color: var(--app-text);">{row.label}</p>
										<p class="text-xs" style="color: var(--app-text-muted);">{row.count} ({row.percent}%)</p>
									</div>
		...
		{:else}
			<div class="mt-3 space-y-3 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
				<div class="flex flex-wrap items-center justify-between gap-2">
					<p class="text-sm font-semibold" style="color: var(--app-text);">Response {selectedResponseIndex + 1} of {totalResponses}</p>
					<div class="flex items-center gap-2">
						<Button variant="secondary" size="sm" onclick={() => (selectedResponseIndex = Math.max(0, selectedResponseIndex - 1))} disabled={selectedResponseIndex <= 0}>Previous</Button>
						<Button variant="secondary" size="sm" onclick={() => (selectedResponseIndex = Math.min(totalResponses - 1, selectedResponseIndex + 1))} disabled={selectedResponseIndex >= totalResponses - 1}>Next</Button>
					</div>
				</div>
				{#if selectedSubmission}
					<div class="rounded-lg border p-3 text-sm" style="border-color: var(--app-glass-border); background: color-mix(in srgb, var(--app-glass-bg) 60%, transparent);">
						<p class="font-medium" style="color: var(--app-text);">{selectedSubmission.profile?.full_name || selectedSubmission.profile?.email || selectedSubmission.user_id}</p>
						<p class="text-xs" style="color: var(--app-text-muted);">Submitted {new Date(selectedSubmission.submitted_at).toLocaleString()}</p>
					</div>
					<div class="space-y-2">
						{#each responseQuestions as q, idx (q.id)}
							<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: color-mix(in srgb, var(--app-glass-bg) 60%, transparent);">
								<p class="text-xs" style="color: var(--app-text-muted);">Q{idx + 1}</p>
								<p class="text-sm font-medium" style="color: var(--app-text);">{q.prompt}</p>
								<p class="mt-1 text-sm" style="color: var(--app-text);">{answerDisplay((selectedSubmission.answers ?? {})[q.id])}</p>
							</div>
						{/each}
					</div>
				{/if}
