...
<section class="space-y-5">
	<div class="fade-up flex items-center justify-between">
		<div>
			...
			<h1 class="text-2xl font-bold tracking-tight">
				<span class="gradient-text">Forms Management</span>
			</h1>
			<p class="mt-1 text-sm" style="color: var(--app-text-muted);">
				Create forms and manage student submissions in one place.
			</p>
		</div>
	</div>
	...
	<div class="fade-up grid gap-4 lg:grid-cols-2" style="animation-delay: 0.05s;">
		<form
			...
			class="space-y-3 rounded-2xl border p-5 backdrop-blur-xl"
		...
		>
			...
			>
			<label class="flex flex-col gap-1 text-sm"
				><span style="color: var(--app-text);">Slug (optional)</span><input
					name="slug"
					class={gi}
					style={gs}
				/></label
			...
			<div class="flex flex-wrap gap-3 text-sm" style="color: var(--app-text);">
				...
				>
				<label class="inline-flex items-center gap-2"
					><input type="checkbox" name="allow_student_view_submissions" checked />Allow students to
					view submissions</label
				>
			</div>
					...
					>
						...
						<div class="grid gap-2 md:grid-cols-2">
							<label class="flex flex-col gap-1 text-xs"
								><span style="color: var(--app-text-muted);">Name</span><input
									name="name"
									class={gi + ' py-1'}
									style={gs}
									value={type.name}
							...
							>
							<label class="flex flex-col gap-1 text-xs"
								><span style="color: var(--app-text-muted);">Slug</span><input
									name="slug"
									class={gi + ' py-1'}
									style={gs}
									value={type.slug}
						...
						<label class="flex flex-col gap-1 text-xs"
							><span style="color: var(--app-text-muted);">Description</span><textarea
								name="description"
								rows="2"
								class={gi + ' py-1'}
								style={gs}>{type.description ?? ''}</textarea
							></label
						>
						<label class="flex flex-col gap-1 text-xs"
							><span style="color: var(--app-text-muted);">Template Google Drive link</span><input
								name="template_drive_link"
								type="url"
								class={gi + ' py-1'}
								style={gs}
								value={type.template_drive_link ?? ''}
							/></label
						>
						<div class="flex flex-wrap gap-3 text-xs" style="color: var(--app-text);">
							...
							<label class="inline-flex items-center gap-2"
								><input
									...
									checked={Boolean(type.allow_student_view_submissions)}
	...
	<GlassCard>
		<div class="flex flex-wrap items-center gap-2">
			...
			<input
				class={gi + ' text-sm'}
				style={gs}
				placeholder="Search student/type/file..."
				bind:value={query}
			/>
			...
			<label class="inline-flex items-center gap-2 text-sm" style="color: var(--app-text);"
				><input type="checkbox" bind:checked={groupedByStudent} />Group by student</label
			>
			...
			<div class="mt-3 space-y-3">
				{#each grouped as group (group.userId)}
					<div
						...
						style="border-color: var(--app-glass-border); background: var(--app-surface-alt);"
					>
						<p class="mb-2 font-medium" style="color: var(--app-text);">
							{group.user?.full_name || group.user?.email || group.userId}
						</p>
						<div class="space-y-2">
							{#each group.rows as row (row.id)}
								...
								>
									<div class="flex flex-wrap items-center gap-2">
										<input
											type="checkbox"
											checked={Boolean(selected[String(row.id)])}
											onchange={(e) =>
												(selected[String(row.id)] = (e.currentTarget as HTMLInputElement).checked)}
										/>
										<p class="font-medium" style="color: var(--app-text);">
											{formById.get(String(row.form_type_id))?.name || row.form_type_id}
										</p>
										<span
									...
									<div class="mt-1 flex flex-wrap items-center gap-2">
										<a
											href={row.file_data_url}
											download={row.file_name}
											class="glass-link rounded-lg border px-2 py-1 text-xs"
											style="border-color: var(--app-glass-border);">{row.file_name}</a
										>
										<form
											method="POST"
										...
										>
											...
											<select name="status" class={gi + ' py-1 text-xs'} style={gs}>
												<option value="submitted" selected={row.status === 'submitted'}
													>submitted</option
												>
												<option value="reviewed" selected={row.status === 'reviewed'}
													>reviewed</option
												>
