...
<section class="space-y-6">
	<header class="fade-up">
		...
		<p class="mt-1 text-sm" style="color: var(--app-text-muted);">
			Scan a student's passport QR to view their profile and certifications.
		</p>
	</header>
	...
	<div class="grid gap-6 lg:grid-cols-[1fr_22rem]">
		<!-- Scanner card -->
		<div
			class="fade-up relative overflow-hidden rounded-2xl border backdrop-blur-xl"
		...
		<div class="space-y-4">
			{#if error}
				<div
					class="fade-up relative overflow-hidden rounded-2xl border p-5"
					style="background: color-mix(in srgb, var(--app-danger) 10%, transparent); border-color: color-mix(in srgb, var(--app-danger) 30%, transparent); box-shadow: 0 0 24px -8px color-mix(in srgb, var(--app-danger) 25%, transparent);"
				>
					<div
						class="pointer-events-none absolute inset-0 rounded-2xl"
						style="background: var(--app-glass-shine);"
					...
					<div class="relative">
						<span
							class="chip-rose inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold"
							>Error</span
						>
			...
			{:else if result}
				<div
					class="fade-up relative overflow-hidden rounded-2xl border p-5 backdrop-blur-xl"
					style="background: var(--app-glass-bg); border-color: var(--app-glass-border); box-shadow: var(--app-glass-shadow);"
				>
					<div
						class="pointer-events-none absolute inset-0 rounded-2xl"
							...
							<div>
								...
								<div class="space-y-1.5">
									{#each result.certifications ?? [] as cert}
										<div
											class="chip-emerald flex items-center gap-2 rounded-lg border px-3 py-1.5 text-sm font-medium"
										>
											<svg
			...
			{:else}
				<div
					class="fade-up rounded-2xl border border-dashed p-6 text-center backdrop-blur-xl"
					style="border-color: var(--app-glass-border); background: var(--app-glass-bg);"
				>
					<p class="text-sm" style="color: var(--app-text-dim);">
						Scan a student QR to view their info.
					</p>
				</div>
			{/if}
		</div>
