...
<section class="space-y-6">
	...
	>
		...
		{#if data.filter.q || data.filter.subteam}
			<a href="/mentor/courses" class="rounded border border-slate-700 px-3 py-2 text-sm">Reset</a>
		{/if}
	</form>

	<div class="overflow-hidden rounded-xl border border-slate-800 bg-slate-900">
		<table class="w-full text-sm">
			<thead class="bg-slate-800/60 text-left text-xs uppercase text-slate-400">
				<tr>
			...
			<tbody>
				{#each data.nodes as node (node.id)}
					<tr class="border-t border-slate-800 hover:bg-slate-800/40">
						...
						<td class="px-3 py-2">T{node.tier}</td>
				...
				{:else}
					<tr>
						<td colspan="6" class="px-3 py-6 text-center text-slate-400">No courses found.</td>
					</tr>
				{/each}
			</tbody>
		</table>
