...
<section class="space-y-6">
	...
	<header class="fade-up flex items-end justify-between">
		<div>
			<p class="eyebrow-label">
				<a href="/team" class="underline" style="color: var(--app-accent);">{data.teamGroup?.name ?? 'Team'}</a> · Subteam
			</p>
			<h1 class="text-2xl font-bold tracking-tight"><span class="gradient-text">{data.subteamCategory?.name ?? data.subteam?.name ?? 'Subteam'}</span></h1>
			{#if !data.userIsOnSubteam}
				<p class="mt-1 text-xs" style="color: var(--app-warning);">You're viewing this subteam but not assigned to it.</p>
			{/if}
		</div>
		<a href={`/courses/map?scope=team:${data.subteam?.id ?? ''}`} class="text-xs font-semibold rounded-lg border px-3 py-1.5" style="background: transparent; border-color: var(--app-glass-border); color: var(--app-text-muted);">Open full graph →</a>
	</header>
	...
	<div class="fade-up space-y-3" style="animation-delay: 0.1s;">
		...
		{#if total === 0}
			<GlassCard><p class="text-sm" style="color: var(--app-text-muted);">No courses tied to this subteam yet.</p></GlassCard>
		{/if}
	</div>
	...
	<div class="fade-up" style="animation-delay: 0.15s;">
		<div class="flex items-end justify-between">
			<div>
				<p class="text-xs uppercase tracking-[0.18em]" style="color: var(--app-text-muted);">Subteam Notes</p>
				<p class="text-xs" style="color: var(--app-text-dim);">{data.canEditNotes ? 'Editable by subteam leads' : 'Posted by subteam leads'}</p>
			</div>
		...
		<div class="mt-2 rounded-2xl border p-4" style="background: var(--app-glass-bg); border-color: var(--app-glass-border);">
			{#if editingNotes}
				<form method="POST" action="?/saveNotes" class="space-y-2">
					...
					<textarea name="body" bind:value={notesDraft} rows="8" placeholder="Notes for this subteam — links, meeting times, tooling, anything subteam members should see." class="w-full rounded-lg border bg-transparent px-3 py-2 text-sm" style="border-color: var(--app-glass-border); color: var(--app-input-text);"></textarea>
	...
	{#if data.canViewRoster && data.roster && data.roster.length > 0}
		<div class="fade-up" style="animation-delay: 0.2s;">
			<div>
				<p class="text-xs uppercase tracking-[0.18em]" style="color: var(--app-text-muted);">Subteam Roster</p>
				<p class="text-xs" style="color: var(--app-text-dim);">{data.roster.length} member{data.roster.length !== 1 ? 's' : ''}</p>
			</div>
