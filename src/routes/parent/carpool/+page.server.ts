...
export const load: PageServerLoad = async ({ locals, url }) => {
	...
	const { data: roles } = dayIds.length
		? await service.from('carpool_day_roles').select('*').in('day_id', dayIds).order('sort_order')
		: { data: [] as any[] };
...
export const actions: Actions = {
	signUp: async ({ locals, request }) => {
		...
		const { selectedStudent } = await resolveParentStudentContext(service, user.id, studentUserId);
		if (!selectedStudent) return fail(403, { error: 'Student is not linked to your parent account.' });
		const { error } = await service.from('carpool_signups').insert({
			role_id: roleId,
