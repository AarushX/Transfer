...
<section class="space-y-6">
	<header class="fade-up">
		...
		<p class="mt-1 text-sm" style="color: var(--app-text-muted);">
			Complete this onboarding course application to register for Walton Robotics.
		</p>
	</header>
	...
	>
		...
		>
			<p class="font-semibold">
				Parental involvement is essential. Parent commitment is required for Walton Robotics
				participation.
			</p>
			<p class="mt-2 text-xs" style="color: var(--app-text-muted);">
				Coach Heather Guiendon letter and expectations are included in this application course. You
				must review and acknowledge before submission.
			</p>
		</div>
		...
		>
			...
			<div class="mt-3 grid gap-3 md:grid-cols-2">
				<label class="space-y-1 text-sm"
					><span style="color: var(--app-text);">I&apos;ve read the introductory letter</span
		...
		>
			<p class="text-sm font-semibold" style="color: var(--app-text);">
				Volunteer Choices (choose at least 3 Yes/Maybe)
			</p>
		...
		<div class="flex flex-wrap items-center justify-between gap-2">
			<p class="text-xs" style="color: var(--app-text-muted);">
				Status: {data.application?.status ?? 'not submitted'}
				{#if data.application?.reviewed_at}(reviewed){/if}
			</p>
	...
	<GlassCard
		...
		subtitle="Enter the link code from your student's profile to connect accounts."
	>
		<form method="POST" action="?/linkStudentByCode" class="space-y-3">
			<label class="block space-y-1 text-sm"
				><span style="color: var(--app-text);">Student link code</span><input
					name="code"
					required
					class={gi + ' uppercase'}
					style={gs}
			...
			<button
				...
				style="background: var(--app-button-secondary-bg); color: var(--app-button-secondary-text); border: 1px solid var(--app-button-secondary-border);"
				>Link student</button
			>
