	...
	<form method="POST" action="?/submitApplication" class="fade-up space-y-4 rounded-2xl border p-5 backdrop-blur-xl" style="background: var(--app-glass-bg); border-color: var(--app-glass-border); box-shadow: var(--app-glass-shadow); animation-delay: 0.05s;">
		...
		<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			...
			<div class="mt-3 grid gap-3 md:grid-cols-2">
				...
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Rookie year on team</span><select name="rookie_year" required class={gi} style={gs}><option value="">Select</option><option value="2023" selected={app.rookie_year === '2023'}>2023</option><option value="2024" selected={app.rookie_year === '2024'}>2024</option><option value="2025" selected={app.rookie_year === '2025'}>2025</option><option value="2026" selected={app.rookie_year === '2026'}>2026 - My student will be a rookie this year!</option></select></label>
			</div>
		</div>

		<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			<p class="text-sm font-semibold" style="color: var(--app-text);">Parent/Guardian 1 Information</p>
			<div class="mt-3 grid gap-3 md:grid-cols-2">
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">FIRST name</span><input name="parent_first_name" required value={app.parent_first_name ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">LAST name</span><input name="parent_last_name" required value={app.parent_last_name ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Relationship</span><input name="relationship" required value={data.application?.relationship ?? app.relationship ?? ''} class={gi} style={gs} placeholder="Parent / Guardian" /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Cell phone</span><input name="phone" required value={data.application?.phone ?? app.phone ?? ''} class={gi} style={gs} placeholder="404.404.4040" /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Email</span><input name="parent1_email" type="email" required value={app.parent1_email ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Occupation</span><input name="parent1_occupation" value={app.parent1_occupation ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Employer</span><input name="parent1_employer" value={app.parent1_employer ?? ''} class={gi} style={gs} /></label>
			</div>
		</div>

		<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			<p class="text-sm font-semibold" style="color: var(--app-text);">Parent/Guardian 2 Information</p>
			<p class="mt-1 text-xs" style="color: var(--app-text-muted);">Optional — fill in if a second parent/guardian is involved.</p>
			<div class="mt-3 grid gap-3 md:grid-cols-2">
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">FIRST name</span><input name="parent2_first_name" value={app.parent2_first_name ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">LAST name</span><input name="parent2_last_name" value={app.parent2_last_name ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Cell phone</span><input name="parent2_phone" value={app.parent2_phone ?? ''} class={gi} style={gs} placeholder="404.404.4040" /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Email</span><input name="parent2_email" type="email" value={app.parent2_email ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Occupation</span><input name="parent2_occupation" value={app.parent2_occupation ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Employer</span><input name="parent2_employer" value={app.parent2_employer ?? ''} class={gi} style={gs} /></label>
			</div>
		</div>
		...
		<div class="space-y-2 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			<p class="text-sm font-semibold" style="color: var(--app-text);">Leadership (Returning Members ONLY)</p>
			...
			{#each [['leadership_parent_volunteer_coord', 'Parent volunteer coordination and parent communication'], ['leadership_sponsorships', 'Getting team sponsorships'], ['leadership_de_field_coordinator', 'DE field coordinator'], ['leadership_field_build_chair', 'Field build chair'], ['leadership_food_committee', 'Food committee chairperson'], ['leadership_banquet_chair', 'End of year banquet chairperson'], ['leadership_shadow_role', 'Shadow a role for future leadership']] as [key, label]}
				<label class="grid gap-1 text-sm md:grid-cols-[1fr_220px] md:items-center"><span style="color: var(--app-text);">{label}</span><select name={key} class={gi} style={gs}><option value="">Select</option><option value="yes" selected={app[key] === 'yes'}>Yes</option><option value="no" selected={app[key] === 'no'}>No</option><option value="maybe" selected={app[key] === 'maybe'}>Maybe - contact me</option></select></label>
			{/each}
		</div>

		<div class="space-y-2 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			<p class="text-sm font-semibold" style="color: var(--app-text);">Skills / Talents / Interest Areas</p>
		...
		<div class="space-y-2 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			...
			<p class="text-xs" style="color: var(--app-text-muted);">Select areas where you could mentor students.</p>
		...
		<div class="space-y-2 rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			<p class="text-sm font-semibold" style="color: var(--app-text);">Commitment Acknowledgements</p>
			<p class="text-xs" style="color: var(--app-text-muted);">Walton Robotics is a varsity-sport time commitment. Please confirm each statement.</p>
		...
		<div class="rounded-lg border p-3" style="border-color: var(--app-glass-border); background: var(--app-surface-alt);">
			...
			<div class="mt-3 grid gap-3 md:grid-cols-2">
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Student name (typed signature)</span><input name="student_signature_name" required value={app.student_signature_name ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Student email (typed signature)</span><input name="student_signature_email" type="email" required value={app.student_signature_email ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Date student signed</span><input name="student_signature_date" type="date" required value={app.student_signature_date ?? ''} class={gi} style={gs} /></label>
				<div></div>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Parent name (typed signature)</span><input name="parent_signature_name" required value={app.parent_signature_name ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Parent email (typed signature)</span><input name="parent_signature_email" type="email" required value={app.parent_signature_email ?? ''} class={gi} style={gs} /></label>
				<label class="space-y-1 text-sm"><span style="color: var(--app-text);">Date parent signed</span><input name="parent_signature_date" type="date" required value={app.parent_signature_date ?? ''} class={gi} style={gs} /></label>
			</div>
		</div>
