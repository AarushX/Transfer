...
export const load: PageServerLoad = async ({ locals }) => {
	...
	const [
		...
		service
			...
			.select('*,category:volunteer_categories(id,name,slug,unit)')
			.eq('season_id', season.id)
			.eq('is_active', true)
			.order('event_date'),
		service
			...
			.select(
				'*,family:families(id,name),signer:profiles!volunteer_signups_user_id_fkey(full_name,email)'
			)
	...
	const mySignupOppIds = new Set(
		(allSignups ?? [])
			.filter((s: any) => s.family_id === family.id)
...
export const actions: Actions = {
	...
	signUp: async ({ locals, request }) => {
		...
		if (filledSlots + slotsClaimed > opp.slots) {
			return fail(400, {
				error: `Not enough slots remaining. Requested: ${slotsClaimed}, Remaining: ${opp.slots - filledSlots}`
			});
		}
