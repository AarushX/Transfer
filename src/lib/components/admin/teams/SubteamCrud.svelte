...
<div class="grid gap-4 xl:grid-cols-3">
	...
	>
		...
		<button class="rounded bg-yellow-400 px-3 py-2 text-sm font-semibold text-slate-900"
				...
				{#each teamGroups as team}
					<label class="flex items-center gap-2 py-0.5">
						<input
							...
							value={team.id}
							checked={linkedGroupIdsBySubteam
								.get(String(selectedSubteam.id))
								?.has(String(team.id)) ?? false}
						/>
						{team.name}
					</label>
				{/each}
			</div>
			<button class="rounded bg-slate-200 px-3 py-2 text-sm font-semibold text-slate-900"
				>Save</button
			>
		</form>
	{:else}
		<div class="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm text-slate-400">
			Select a subteam to edit.
		</div>
	{/if}
	...
	{#if selectedSubteam}
		<form
			...
			class="rounded-xl border border-red-900/60 bg-red-950/20 p-4"
		>
			<input type="hidden" name="subteam_id" value={selectedSubteam.id} />
			<p class="text-sm text-red-200">Delete subteam</p>
			<p class="mt-1 text-xs text-red-300/80">
				This removes subteam assignments and mappings that depend on it.
			</p>
			<button
				class="mt-3 rounded bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-500"
				>Delete subteam</button
			>
		</form>
	{:else}
		<div class="rounded-xl border border-slate-800 bg-slate-900 p-4 text-sm text-slate-400">
			Select a subteam to delete.
		</div>
	{/if}
</div>
