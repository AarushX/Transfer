...
<div class="space-y-3 rounded-xl border border-slate-800 bg-slate-900 p-4">
	...
	<div class="grid gap-2">
		{#each categories as category}
			<div
				class="grid gap-2 rounded border border-slate-800 bg-slate-950/40 p-2 lg:grid-cols-[1fr_auto]"
			>
				<form
					method="POST"
					action="?/updateCategory"
					class="grid gap-2 md:grid-cols-[1fr_120px_180px_100px]"
				>
				...
				<form method="POST" action="?/deleteCategory">
					<input type="hidden" name="category_slug" value={category.slug} />
					<button
						class="rounded bg-red-600 px-3 py-2 text-sm font-semibold text-white hover:bg-red-500"
						>Delete</button
					>
				</form>
			</div>
